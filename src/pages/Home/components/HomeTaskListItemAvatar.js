...
const HomeTaskListItemAvatar = ({ task, users, onChangeUser }) => {
  ...
  return (
    ...
    >
      <div 
        className='todo-home__task-item-avatar d-flex align-items-center'
        onClick={handleClickShowDropDown}
      >
        <figure className='todo-home__task-item-img-container'>
          <img 
            className='todo-home__task-item-img'
        ...
        <div className='todo-home__task-item-email ml-2'>
          {task.user.email}
        </div>
        <i className='todo-icon-arrow_drop_down-24px todo-home__task-item-icon' />
      </div>
      <div 
        className={`todo__dropdown-content ${showDropDown ? 'todo__dropdown-content--show' : ''}`}
      >
        {users.usersList.items.map(user => <HomeDropDownItem 
          user={user} 
          key={user.id} 
          onChangeUser={onChangeUser}
          onClickShowDropDown={handleClickShowDropDown}
        />)}
      </div>
  </div>
  );
...
export default HomeTaskListItemAvatar;
